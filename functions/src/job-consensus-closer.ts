 ...
 */
export const onJobCallFeedbackWritten = functions.firestore
  ...
  .onWrite(async (change, context) => {
    ...
    try {
      ...
      functions.logger.info(
        `Job ${jobId} has ${distinctWorkers.size} distinct worker reports of FILLED.`
      );
      ...
      if (distinctWorkers.size >= 3) {
        ...
        functions.logger.info(
          `Consensus reached (>=3 reports)! Auto-closing job ${jobId} to prevent spam calls.`
        );
        ...
        await Promise.all([
          ...
          db.collection("jobmetadata").doc(jobId).set({ ...closePayload, status: "FILLED" }, { merge: true }),
