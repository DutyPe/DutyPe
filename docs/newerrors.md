...
StateFlow + collectAsStateWithLifecycle is used in most places.
...
Replace all collectAsState() → collectAsStateWithLifecycle()
Fix var mutable properties on @Stable JobListing
Add @Immutable to all UiState and filter data classes
Move Firestore calls out of MainNavGraph into a dedicated AppStartupViewModel
Fix hardcoded Telugu strings → res/values-te/strings.xml
P1 — Before 10M Users
Add kotlinx.collections.immutable and use ImmutableList<JobListing> everywhere
Upgrade Navigation to 2.9.x with type-safe @Serializable destinations
Extract Use Cases from AllJobsViewModel and FirestoreJobViewModel
Add Paging 3 with a Firestore PagingSource
Split PostJobScreen.kt, MandatoryWorkerProfileSetupScreen.kt into composable packages
P2 — Scale Architecture
Introduce feature modules (start with :feature:auth, :feature:jobs)
Write ViewModel unit tests for AllJobsViewModel and navigation logic
Add @Preview to all public composables
Implement full Dark Mode (remove ForceLightTheme)
Rename workers/ → background/ to eliminate naming confusion
REWRITE EXAMPLES
