...
type EventMessage = { key: string; recipientId: string; title: string; message: string; type: string; targetRole: string; data: Record<string, string> };

async function persistEvents(transaction: admin.firestore.Transaction, events: EventMessage[]): Promise<number> {
  if (events.length === 0) return 0;
  const refs = events.map(event => db.collection("notification_events").doc(createHash("sha256").update(event.key).digest("hex")));
  const receipts = await transaction.getAll(...refs);
  let created = 0;
  const now = Date.now();
  events.forEach((event, index) => {
    if (receipts[index].exists) return;
    const { key, ...notification } = event;
    const id = refs[index].id;
    transaction.create(refs[index], { eventKey: key, notificationId: id, createdAt: now });
    transaction.create(db.collection("notifications").doc(id), {
      ...notification, id, createdAt: now, expiresAt: now + retention, isRead: false
    });
    created += 1;
  });
  return created;
...
export async function emitApplicationNotifications(applicationId: string, requesterId?: string, requestedRecipient?: string, reminder = false): Promise<number> {
  return db.runTransaction(async transaction => {
    ...
    if (!statusTitles[status]) throw new functions.https.HttpsError("failed-precondition", "Unknown application state");
    ...
    const event = (recipientId: string, targetRole: string, type: string, title: string): EventMessage => ({
    ...
    if (!reminder && ["PENDING", "ACCEPTED", "WITHDRAWN"].includes(status)) {
      events.push(event(application.employerId, "EMPLOYER", status === "PENDING" ? "NEW_APPLICATION" : "APPLICATION_STATUS",
        status === "PENDING" ? "New Application Received" : statusTitles[status]));
    }
    return persistEvents(transaction, events);
  });
}

export const notifyApplicationEvent = functions.firestore.document("job_applications/{applicationId}").onWrite(async (change, context) => {
  if (!change.after.exists || (change.before.exists && change.before.get("status") === change.after.get("status"))) return;
  await emitApplicationNotifications(context.params.applicationId);
});

export const requestNotification = functions.https.onCall(async (data, context) => {
  const userId = context.auth?.uid;
  if (!userId) throw new functions.https.HttpsError("unauthenticated", "Sign in to request a notification");
  if (data?.userId !== undefined && data.userId !== userId) throw new functions.https.HttpsError("permission-denied", "Account changed");
  const type = validateString(data?.type, "type", { required: true, maxLength: 50 });
  const details = data?.data && typeof data.data === "object" ? data.data : {};
  const recipientId = validateString(data.recipientId, "recipientId", { required: true, maxLength: 128, pattern: /^[a-zA-Z0-9_-]+$/ });
  ...
  if (applicationTypes.includes(type)) {
    const applicationId = validateString(details.applicationId, "applicationId", { required: true, maxLength: 256, pattern: /^[a-zA-Z0-9_-]+$/ });
    const created = await emitApplicationNotifications(applicationId, userId, recipientId,
      details.notificationType === "PENDING_APPLICATION_REMINDER" || type === "APPLICATION_REMINDER");
    return { success: true, created };
  }
  if (type === "WORKER_HIRED") {
    const jobId = validateString(details.jobId, "jobId", { required: true, maxLength: 256, pattern: /^[a-zA-Z0-9_-]+$/ });
    let query: admin.firestore.Query = db.collection("job_applications")
      .where("employerId", "==", userId).where("jobId", "==", jobId).where("status", "in", ["ACCEPTED", "IN_PROGRESS", "COMPLETED"]);
    if (recipientId !== userId) query = query.where("workerId", "==", recipientId);
    const applications = await query.limit(100).get();
    if (applications.empty) throw new functions.https.HttpsError("permission-denied", "No matching hire belongs to this employer");
    let created = 0;
    for (const application of applications.docs) created += await emitApplicationNotifications(application.id, userId, recipientId);
    return { success: true, created };
  }
  if (recipientId !== userId) throw new functions.https.HttpsError("permission-denied", "This notification can only target your own account");
  ...
  const created = await db.runTransaction(async transaction => {
    ...
    const eventData: Record<string, string> = { deepLink: targetRole === "EMPLOYER" ? "dutype://employer/home" : "dutype://worker/home" };
    if (type === "JOB_POSTED" || type === "JOB_PAUSED") {
      const jobTitle = validateString(details.jobTitle, "jobTitle", { required: true, maxLength: 500 });
      const jobs = await transaction.get(db.collection("jobs").where("employerId", "==", userId).where("title", "==", jobTitle).limit(1));
      if (jobs.empty) throw new functions.https.HttpsError("permission-denied", "Job does not belong to your account");
      ...
      if (!date || Number(date[1]) !== today.getUTCMonth() + 1 || Number(date[2]) !== today.getUTCDate()) return 0;
