 ...
 */
export const workerLanding = functions.https.onRequest(async (req, res) => {
  try {
    ...
    // Create description
    const description = `${workerName} - ${primarySkill}${isVerified ? " ✓ Verified" : ""}${
      completedJobs > 0 ? ` | ${completedJobs} jobs completed` : ""
    ...
    <div class="container">
        ...
        ${
          skillsList.length > 0
            ? `
        <div class="skills">
            ${skillsList.map((skill: string) => `<div class="skill">${skill}</div>`).join("")}
        </div>
        `
            : ""
        }
