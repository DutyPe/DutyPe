            ...
            for (const message of messages) {
              ...
              // Handle Interactive Button Reply (Employer Check-in & Stop Calls)
                ...
                if (buttonId.startsWith("HIRED_STOP_")) {
                  ...
                  const reply =
                    "🎉 Mubarak! Candidate marked as HIRED on DutyPe.\n\n" +
                  ...
                  const reply =
                    "🎉 Candidate marked as HIRED on DutyPe!\n\n" +
                  ...
                  if (!checkinSnap.empty) {
                    ...
                    if (cData.jobId) {
                      ...
                      await sendWhatsAppMessage(
                        from,
                        "✅ Your job has been REOPENED! Workers can now call you again.",
                        phoneNumberId,
                        accessToken
                      );
                      continue;
                  ...
                  if (!pendingCheckinSnap.empty) {
                    ...
                    if (isOption1) {
                      if (checkin.applicationId) {
                        await db.collection("applications").doc(checkin.applicationId).set(
                          { status: "HIRED", notes: "Hired via WhatsApp (calls stopped)", updatedAt: admin.firestore.FieldValue.serverTimestamp() },
                          { merge: true }
                        );
                      }
                      if (checkin.jobId) {
                      ...
                      await sendWhatsAppMessage(
                        from,
                        "🎉 Mubarak! Candidate marked as HIRED. Incoming calls STOPPED. Reply REOPEN anytime if you need to hire again.",
                        phoneNumberId,
                        accessToken
                      );
                      continue;
                      ...
                      if (checkin.applicationId) {
                        await db.collection("applications").doc(checkin.applicationId).set(
                          { status: "HIRED", notes: "Hired via WhatsApp (keep open)", updatedAt: admin.firestore.FieldValue.serverTimestamp() },
                          { merge: true }
                        );
                      }
                      ...
                      await sendWhatsAppMessage(
                        from,
                        "🎉 Candidate marked as HIRED! Your job remains OPEN for more calls.",
                        phoneNumberId,
                        accessToken
                      );
                      continue;
                      ...
                      if (checkin.applicationId) {
                        await db.collection("applications").doc(checkin.applicationId).set(
                          { status: "REJECTED", notes: "Not suitable via WhatsApp", updatedAt: admin.firestore.FieldValue.serverTimestamp() },
                          { merge: true }
                        );
                      }
                      ...
                      await sendWhatsAppMessage(
                        from,
                        "Got it! Feedback recorded. We'll keep sending you more qualified candidates.",
                        phoneNumberId,
                        accessToken
                      );
                      continue;
...
export async function sendEmployerWhatsAppCheckIn(
  toPhone: string,
  workerName: string,
  jobTitle: string,
  jobId: string,
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const fallbackText =
        `Namaste! 🙏 Yesterday *${workerName}* contacted you regarding *${jobTitle}* on DutyPe.\n\n` +
